...
public class GameState implements Cloneable {
    private Boolean[][] pieces; // true = red, false = yellow, null = empty
    private Stack<Point> moves;
    private String error;
    private boolean gameOver;
    private boolean redWins;
    ...
    public boolean move(int column) {
        ...
        if (gameOver || column < 1 || column > 7)
            return false;

        int col = column - 1;

        // Colonne pleine ?
        if (pieces[col][5] != null) {
            return false;
        }

        int row = 0;
        while (row < 6 && pieces[col][row] != null) {
            row++;
        }
            ...
            for (int y = 0; y < 6; y++) {
                for (int[] dir : directions) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        if (cx >= 0 && cx < 7 && cy >= 0 && cy < 6) {
                            Boolean piece = pieces[cx][cy];
    ...
    public boolean isDraw() {
        return gameOver && !redWins && !yellowWins;
    }
    ...
    public GameState clone() {
        try {
            ...
            // Cloner en profondeur les tableaux et les stacks si nécessaire
            cloned.pieces = new Boolean[7][6];
            for (int i = 0; i < 7; i++) {
                for (int j = 0; j < 6; j++) {
                    cloned.pieces[i][j] = this.pieces[i][j];
                }
            }
            cloned.moves = (Stack<Point>) this.moves.clone();
            return cloned;
        } catch (CloneNotSupportedException e) {
            throw new AssertionError(); // Ne peut pas arriver, car on implémente Cloneable
        }
    }
